...
export default function CardTable({ color }) {
  ...
  const [records, isPending, error, refreshData] = useFetch(
    'http://localhost:3002/real_estate'
  )
  ...
  const handleGetWard = async (discode, wardcode) => {
    const resWard = await fetch(`http://localhost:3002/wards/${discode}`)
  ...
  const handleGetDis = async (proCode, discode, wardcode) => {
    const resDis = await fetch(`http://localhost:3002/districts/${proCode}`)
